...
type BucketCount = { siteId: string; openPositions: number; oldestDaysOpen: number };

// Open-count for one bucket of roles (used for both strategic sites and the two
// unmapped buckets — the latter feed the map's cached badge fallback).
function countFor(roles: Role[], siteId: string): BucketCount {
  const hits = roles.filter((r) => r.siteId === siteId);
  const oldest = hits.reduce((mx, r) => (r.daysOpen != null && r.daysOpen <= STANDING_DAYS ? Math.max(mx, r.daysOpen) : mx), 0);
  return { siteId, openPositions: hits.length, oldestDaysOpen: oldest };
}

function aggregate(roles: Role[]): { sites: SnapshotSite[]; unmapped: BucketCount[]; totalOpen: number } {
  const sites = SITES.map(({ match: _m, ...site }) => ({ ...site, ...countFor(roles, site.siteId) }))
    .sort((a, b) => b.openPositions - a.openPositions || b.oldestDaysOpen - a.oldestDaysOpen);
  // The two unmapped buckets — kept out of `sites` so strategicOpen/otherOpen hold,
  // but cached so the map badges fall back to a real count instead of zeroing.
  const unmapped = ["us-retail", "eu-other"].map((id) => countFor(roles, id));
  return { sites, unmapped, totalOpen: roles.length };
}
...
async function main() {
  ...
  const { sites, unmapped, totalOpen } = aggregate(roles);
  ...
  const cache = {
    _note: "Cached careers snapshot — offline-safe fallback for /api/feeds/careers (aggregate KPIs only). Mirrors the latest Supabase row; auto-generated by scripts/pull-jobs.ts. Roles are LIVE-ONLY (no copy here) — the DB is their single source. `sites` = strategic sites only (so strategicOpen/otherOpen hold); `unmapped` = the two bucket counts (us-retail, eu-other) for the map's cached badge fallback, deliberately NOT in `sites`. other = totalOpen - Σsites = Σunmapped.",
    asOf, source: "careers.st-group.com (SAP SuccessFactors)", totalOpen, crawledAt, sites, unmapped,
  };
